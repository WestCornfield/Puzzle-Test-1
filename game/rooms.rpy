...
screen PuzzleRoomScreen():
    ...
    if wall_smashed:
        imagebutton:
            ...
            at hidden_panel
            action [SensitiveIf(in_room and not inside_option), Jump("BustedWall")]
    if wall_smashed and not 'scroll' in inventory:
        imagebutton:
            idle "scroll_idle"
            at scroll_location
            action [SensitiveIf(in_room and not inside_option), Jump("Scroll")]
    if not scroll_read:
        imagebutton:
            idle "rock_door_closed_idle"
            at door_location
            action [SensitiveIf(in_room and not inside_option), Jump("Door")]
    if scroll_read:
        imagebutton:
    ...
    if not wall_smashed:
        imagebutton:
            ...
            at secret_spot
            action [SensitiveIf(in_room and not inside_option), Jump("SecretSpot")]
    if open_inventory:
        imagebutton:
